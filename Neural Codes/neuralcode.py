...
def compare_all_canonical_groebner(dimension):
    ...
    all_codes = all_neural_codes(dimension)
    failures = 0
    equal = 0
    ...
    for i in range(1, len(all_codes)):
        ...
        if(canonical_form != "Empty"):
            this_comparison = compare_groebner_canonical(groebner, canonical_form)
            factored_groebner = []
            
            for k in range(len(this_comparison[1])):
                factored_groebner.append(this_comparison[1][k].factor())
                
            if (this_comparison[0] == False):
                failures = failures + 1
                different_codes.append(all_codes[i])
                different_groebner.append(factored_groebner)
                different_canonical.append(neural_ideal.factored_canonical())
            else:
                equal = equal + 1
                equal_codes.append(all_codes[i])
                equal_groebner.append(factored_groebner)
                equal_canonical.append(neural_ideal.factored_canonical())

    print "Differences: " + str(failures) + " Equal bases: " + str(equal) + "\n"
    print "Different Bases: \n"
    template = "{0:50}|{1:75}|{2:90}" 
    print template.format("Codes", "Groebner", "Canonical")
    for i in range(failures):
        print template.format(different_codes[i], different_groebner[i], different_canonical[i])
      
    print "\nEqual Bases: \n"
    template = "{0:50}|{1:80}|{2:90}"
    print template.format("Codes", "Groebner", "Canonical")
    for i in range(equal):
        print template.format(equal_codes[i], equal_groebner[i], equal_canonical[i])
            
    
def compare_groebner_canonical(groebner, canonical):
    """
    
    Args:
        groebner : A groebner basis of the neural ideal.
        canonical: The canonical form of the neural ideal.
        
    Returns:
        Returns a list: [Boolean, Groebner Basis, Canonical Form]

    Raises:
        None.
    """
    r"""
    Returns a list whose first element is True/False is the groebner and canonical forms are equal, and whose second/third elements are the groebner and canonical forms, respectively.

    INPUT:

    - ``groebner`` -- ideal The groebner basis of the neural ideal.

    - ``canonical`` -- ideal The canonical form of the neural ideal.

    OUTPUT:

    Comparison as a list: [Boolean, groebner, canonical].
    ...
    """
    canon = Set(canonical.gens())
    groebner = Set(groebner.gens())
    comparison = []
    
    # add in different comparison forms
    canon_size = canon.cardinality()
    groebner_size = groebner.cardinality()
    
    num_shared = 0
    
    for i in range(canon_size):
        if canon[i] in groebner:
            num_shared = num_shared + 1
    
    if canon_size <= groebner_size and canon_size == num_shared:
...
def generate_random_tests(num_of_tests, dimension = randint(2, 20)):
    ...
    Args:
        num_of_tests           : Number of random tests.
        dimension (optional)   : Dimension of the polynomial ring (randomly chosen between 2 and 20, if not specified).

    Returns:
        A list of Groebner/Canonical comparisons.
    ...
    - ``num_of_tests`` -- integer  The number of random tests to run.

    - ``dimension`` -- integer (default: random number between 2 and 20) the dimension of the neural codes to be tested.
    ...
    for i in range(num_of_tests):
        all_codes.append(generate_random_code(dimension))
    ...
    for i in range(len(all_codes)):
        ...
        if(canonical_form != "Empty"):
            this_comparison = compare_groebner_canonical(universal_groebner, canonical_form)
            factored_groebner = []
            ...
            if (this_comparison[0] == False):
                failures = failures + 1
                if len(universal_groebner.gens()) == 1:
                    different_codes.append("*" + str(all_codes[i]))
                else:
                    different_codes.append(all_codes[i])
                different_groebner.append(factored_groebner)
                different_canonical.append(neural_ideal.factored_canonical())
                ...
                equal = equal + 1
                if len(universal_groebner.gens()) == 1:
                    equal_codes.append("*" + str(all_codes[i]))
                else:
                    equal_codes.append(all_codes[i])
                equal_groebner.append(factored_groebner)
                equal_canonical.append(neural_ideal.factored_canonical())
    ...
    print "Differences: " + str(failures) + " Equal bases: " + str(equal) + "\n"
    print "Different Bases: \n"
    template = "{0:50}|{1:80}|{2:80}" 
    print template.format("Codes", "Groebner", "Canonical")  
    for i in range(len(different_codes)):
      print template.format(different_codes[i], different_groebner[i], different_canonical[i])
